...
class CleanCatalogService:
    ...
    def get_module_with_units(self, module_uid: str) -> Optional[ModuleDetails]:
        ...
        """
        # First attempt: URL manifest cache (if enabled)
        try:
            use_manifest = os.getenv('USE_URL_MANIFEST', 'true').lower() in ('1', 'true', 'yes')
        except Exception:
            use_manifest = True

        if use_manifest:
            try:
                from .url_cache import get_url_cache  # Local import to avoid circulars if any
                cache = get_url_cache()
                module_meta = cache.get_module(module_uid)
                if module_meta and not cache.needs_refresh(module_uid):
                    # Build ModuleDetails directly from manifest
                    units = []
                    for u in module_meta.units:
                        units.append(Unit(
                            title=u.title,
                            url=u.url,
                            type=u.type,
                            duration_minutes=u.duration_minutes or 5
                        ))
                    return ModuleDetails(
                        uid=module_meta.uid,
                        title=module_meta.title,
                        summary='',  # Manifest doesn't store summary yet
                        url=module_meta.url,
                        duration_minutes=module_meta.duration_minutes,
                        level=module_meta.level,
                        rating=0.0,
                        units=units
                    )
            except Exception as e:
                logger.debug(f"URL manifest cache lookup failed for {module_uid}: {e}")
        ...
        for module in all_curated_modules:
            if module.uid == module_uid:
                details = self._convert_module_to_details(module)
                # If manifest is enabled and module missing or stale, attempt background refresh (best-effort)
                if use_manifest:
                    try:
                        from .url_cache import get_url_cache
                        cache = get_url_cache()
                        if cache.needs_refresh(module_uid):
                            # Fire and forget thread to refresh
                            import threading
                            def _bg_refresh():
                                try:
                                    cache.refresh_module(uid=module.uid, module_url=module.url, level=module.level, duration_minutes=module.duration_minutes, verify_head=True)
                                except Exception:
                                    pass
    ...
    def _convert_module_to_details(self, module: Module) -> ModuleDetails:
        """Convert a Module object to ModuleDetails.

        IMPORTANT: Synthetic (fake) unit generation has been removed. If real units
        are required they must be populated via the URL manifest / cache layer.
        This method now returns an empty units list when no real data is present
        so downstream processes can decide whether to block, refresh, or skip.
        """
        if module.unit_count:
            logger.warning(
                "Module %s reports unit_count=%s but synthetic unit fallback is disabled; returning empty units list.",
                module.uid, module.unit_count
            )
        units = []
        return ModuleDetails(
            uid=module.uid,
    ...
    def _generate_unit_title(self, module_title: str, unit_number: int) -> str:
        """Deprecated: Previously generated synthetic unit titles. Now unused."""
        return f"Deprecated Unit {unit_number}"
